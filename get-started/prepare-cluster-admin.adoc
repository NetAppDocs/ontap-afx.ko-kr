...
NOTE: ONTAP 9.19.1부터 SVM 관리자는 전용 System Manager 대시보드를 사용하여 자체 SVM을 모니터링하고 관리할 수 있습니다. 에 대해 자세히 알아보십시오 https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html["ONTAP System Manager SVM 관리자 대시보드"^]
...
AFX는 클러스터를 구성하고 사용할 때 최소값과 최대값을 포함한 여러 가지 제한 사항을 적용합니다. 이러한 제한 사항은 여러 범주로 나뉩니다.


CAUTION: AFX 스토리지 시스템의 운영 매개 변수를 늘리기 전에 해당 변경 사항이 애플리케이션과 클러스터 전체에 미치는 영향을 평가해야 합니다. 예를 들어 컨트롤러 노드 수를 늘리면 전체 용량은 증가하지만 클러스터 네트워크 트래픽도 증가할 수 있습니다. 이러한 구성 변경의 의미와 환경에 미칠 수 있는 영향을 이해하는 것이 중요합니다.

클러스터당 컨트롤러 노드:: 각 AFX 클러스터는 최소 4개의 노드를 가져야 합니다. 최대 노드 수는 ONTAP 릴리스 버전에 따라 다르며, 예를 들어 9.19.1 릴리스에서는 AFX 클러스터당 최대 32개의 노드를 허용합니다.
...
NetApp Hardware Universe 및 상호 운용성 매트릭스 도구에서 제공되는 세부 정보를 검토하여 AFX 클러스터의 기능을 확인해야 합니다. 또한 AFX 릴리스 노트 섹션의 link:../release-notes/changes-to-limits-defaults.html["ONTAP 제한 및 기본값 변경 사항"]도 검토하십시오.
